...
do
        ...
        echo "Using sqlite3 to backup system.db to system.db.dump on $NODE"
	SQLOK=false
        for (( t=0; t<3; t++ ))
        do
                #remove previous system.db.dump, may prevent diskIO error
                if [[ -f "$($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db.dump" ]]; then
                        rm -f $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db.dump 2>&1
                fi
                sqlite3 $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db .dump > $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db.dump
                STATUS=$?
                if [[ $STATUS -eq 0 ]]; then
                        #succes, stop trying
                        SQLOK=true
        ...
        if [[ ! $SQLOK ]]; then
                ...
                cp -f $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db.dump
                STATUS=$?
                if [[ $STATUS -eq 0 ]]; then
 		...
 		OUTPUT=$($MAINPATH/restic backup --tag $NODE $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data $NODEBASEPATH/$NODE/.origintrail_noderc $($DOCKER_INSPECT_MERGED $NODE)$ARANGODB3 $($DOCKER_INSPECT_MERGED $NODE)$ARANGODB3APPS --exclude="brick.img" --exclude "bootstraps.json" --exclude "system.db-journal" --exclude "import_cache" --exclude "kadence.dht" --exclude "peercache" --exclude "replication_cache" 2>&1)                STATUS=$?
                if [[ $STATUS -eq 0 ]]; then
                        if [[ $BACKUP_NOTIFY_ON_SUCCESS == "true" ]]; then
                                ...
                                rm $($DOCKER_INSPECT_MERGED $NODE)/ot-node/data/system.db.dump
                        fi
                else
